      ...
      {open && (
        <div className="p-4 grid grid-cols-2 gap-3">
          <div className="col-span-2">
            ...
            <input className="cv-input" placeholder="Nombre de la empresa" value={exp.company} onChange={(e) => updateExperience(exp.id, 'company', e.target.value)} />
          </div>

          <div className="col-span-2">
            <label className="cv-label">Puesto / Rol</label>
            <input className="cv-input" placeholder="Ej: Desarrollador Frontend" value={exp.position} onChange={(e) => updateExperience(exp.id, 'position', e.target.value)} />
          </div>

          <div>
            <label className="cv-label">Fecha inicio</label>
            <input type="month" className="cv-input" value={exp.startDate} onChange={(e) => updateExperience(exp.id, 'startDate', e.target.value)} />
          </div>
